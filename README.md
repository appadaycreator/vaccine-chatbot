...
- **資料にない場合**: 検索結果が0件のときは、LLM生成を行わず固定文（「資料にない」）を返します。検索結果はあるがキーワードフィルタで全件落ちた場合は、検索結果をフォールバックでコンテキストに使い回答を試みます（短い質問でも回答が返りやすくなります）
- **一般論は返さない**: 「一般的には…」のような一般論・推測による補完は行いません
- **根拠の提示**: 回答本文とは別に、UIが `sources`（資料名/ページ/抜粋）を表示します
- **検索フィルタ**: キーワードフィルタと全落ち時フォールバックは `rag_filter` で共通化しており、API・Streamlit・CLIで同じ挙動です
...
- UI では 530 やネットワークエラー時に **画面上部に赤い固定バナー** とチャット欄のシステムメッセージで対処を表示します（「cloudflared トンネルと API サーバーが起動しているか確認してください」）。
- **コンソールに「CORS」「530」と出るのは仕様です**（リクエストが API に届いていないため）。バナーとメッセージで対処内容を案内しています。
...
- **APIのコードを更新したが挙動が変わらない**
  - API（uvicorn）を**再起動**してください。GitHub Pages はフロントのみのため、cloudflared の先などで動かしている API サーバーを再起動する必要があります。既定では検索結果をそのままコンテキストに使うため、再起動後は「概要を教えて」などの短い質問でも回答が返りやすくなります。
- **PDFが参照できていない**
...
- **キーワードフィルタで全件落ちている**
  - 既定では **`SEARCH_KEYWORD_OVERLAP_MIN=0`**（フィルタ無効）のため、検索結果はそのままコンテキストに使われます。厳しめにしたい場合は **`SEARCH_KEYWORD_OVERLAP_MIN=1`** を設定してください（API起動時または `.env`）。**APIを更新した場合はサーバー再起動が必要です**
