...
- **根拠が取れない場合の扱い**:
  - 既定（安全側）: `sources` が空（=参照PDFから該当箇所が取れない）場合は、LLM生成を行わず固定文（「資料にない」）を返します
  - 例外（UX優先）: UIのチェック、またはAPIリクエストで **一般説明フォールバック** を有効にすると、参照PDFに基づくと断定しない前提で一般的な説明を返します（根拠（引用）は表示されないことがあります）
...
- **一般説明フォールバック**: 「資料の該当箇所が見つからない場合も、一般的な説明として回答する」をONにすると、`sources` が空でも回答を返します（参照PDFに基づくと断定しない前提）
...
> 結論: 資料に記載がないため、この資料に基づく回答はできません。（質問: ...）
